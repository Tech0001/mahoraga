...
export const AgentConfigSchema = z.object({
  ...
  momo_scanner_enabled: z.boolean(),
  momo_scan_interval_ms: z.number().min(15000).max(600000),
  momo_min_change_pct: z.number().min(0).max(100),
  momo_min_rvol: z.number().min(0).max(50),
  momo_min_volume: z.number().min(0).max(1e9),
